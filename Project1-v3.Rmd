...
# summary(model2)
# AIC(model2)
cor(train$highway.MPG,train$city.mpg) ##0.9428157
cor(train$Engine.HP,train$Engine.Cylinders)## 0.7859781
...
modelI<-lm(log(MSRP)~Make+Year+Engine.Fuel.Type+Engine.HP+Engine.Cylinders+Transmission.Type+Driven_Wheels+Number.of.Doors+highway.MPG+city.mpg+ Popularity+Crossover+Diesel+Exotic+Luxury+Performance+FactoryTuner+FlexFuel+Hatchback+Hybrid+
          Engine.HP:Engine.Cylinders+highway.MPG:city.mpg,data=train1 )
summary(modelI)
AIC(modelI)
...
```{r}
par(mfrow = c(2,2))
plot(modelI)              s## residual plot is somewhat normal distribution . There is no points that is low residual and high leverage. The model looks good
```
...
```{r}
SSE_I <- sse(Predicted_I_NV$valid, Predicted_I_NV$prediction)
SSE_I

MSE_I <- mse(Predicted_I_NV$valid, Predicted_I_NV$prediction)
MSE_I 

AdjR2_I <- 1-(MSE_Step/var(Predicted_I_NV$valid1))
AdjR2_I
...
```{r}
SSE_K <- sse(KNNPredictedV$valid,KNNPredictedV$prediction)
SSE_K

MSE_K <- mse(KNNPredictedV$valid, KNNPredictedV$prediction)
MSE_K 

AdjR2_K <- 1-(MSE_Step/var(KNNPredictedV$valid))
AdjR2_K
